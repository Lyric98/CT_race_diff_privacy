...
agg_sex_dp19 <- aggregate(dp_pop~GEOID+race+agecat,data=adat, 
                        FUN = sum,na.rm=T)

agg_sex_dp20 <- aggregate(dp0527_pop~GEOID+race+agecat,data=adat, 
                            FUN = sum,na.rm=T)
...
# stdcount & stdpop
std_pm <- read.csv("standard_ga.csv")
...
for (i in unique(adat$GEOID)){
  for (j in unique(adat$race)){
    count = count
    pop_dp = agg_sex_dp19 %>% filter(GEOID ==i, race==j) 
...
# 0527 new dp matrix
dp20_matrix <- matrix(0,ncol = length(unique(adat$GEOID)),
                        nrow=length(unique(adat$race)), 
                        dimnames = list(unique(adat$race),
                                        unique(adat$GEOID)))
for (i in unique(adat$GEOID)){
  for (j in unique(adat$race)){
    count = count
    pop_dp20 = agg_sex_dp20 %>% filter(GEOID ==i, race==j) 
    output <- ageadjust.indirect(count, pop_dp20$dp0527_pop, stdcount, stdpop, stdrate = NULL, conf.level = 0.95)
    dp20_matrix[j, i] <- output$sir[2]
  }
}
...
dp20_l <- data.frame(t(dp20_matrix)) %>% 
  mutate(GEOID = dimnames(data.frame(t(dp20_matrix)))[[1]]) %>% 
  gather(race, exp_counts_dp20, -GEOID)

exp_l <- ce_l %>% left_join(dp_l, by=c("GEOID", "race")) %>% left_join(dp20_l, by=c("GEOID", "race"))
...
adat<-subset(adat,exp_counts_ce>0 & exp_counts_dp>0 & exp_counts_dp20>0)
...
save(cars,file=paste0('garesults/simnum',simnum,'.RData'))
save(exp_theta, file=paste0('garesults/exp_theta',simnum,'.RData'))
