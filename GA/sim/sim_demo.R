...
for (i in unique(adat$GEOID)){
  for (j in unique(adat$race)){
    ...
    dp_matrix[j, i] <- sum(pop_dp$dp_pop)
  }
}
...
for (i in unique(adat$GEOID)){
  for (j in unique(adat$race)){
    ...
    dp0527_matrix[j, i] <- sum(pop_dp0527$dp0527_pop)
  }
}
...
for (i in unique(adat$GEOID)){
  for (j in unique(adat$race)){
    ...
    dp22_matrix[j, i] <- sum(pop_dp22$dp22_pop)
  }
}
