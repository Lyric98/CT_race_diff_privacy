...
```{r scatter plot SMR bias vs ce_exp_counts (only black)}
smr_bias_ce_black <- smr_bias[,c("GEOID", "race","exp_counts_ce", "dp_smr_bias","dp0527_smr_bias")] %>% filter(race=='black')
#[which(smr_mape_box$race=='white'),]
smr_bias_ce_black_long <- melt(smr_bias_ce_black, id=c("GEOID", "race", "exp_counts_ce"))
smr_bias_ce_black_scatter <- ggplot(data = smr_bias_ce_black_long, mapping = aes(x = exp_counts_ce, y = value, color=race)) + geom_point(alpha=0.1) + ylab("SMR bias")+scale_color_brewer(palette="Dark2")+ scale_x_sqrt()
smr_bias_ce_black_scatter + facet_wrap(~variable) + 
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE) + ggtitle("SMR bias vs ce_exp_counts(only black)")


smr_bias_ce_black_scatter_wo_oulier <- ggplot(data = smr_bias_ce_black_long[smr_bias_ce_black_long$GEOID!="25025980101",], mapping = aes(x = exp_counts_ce, y = value, color=race)) + geom_point(alpha=0.3) + ylab("SMR bias")+scale_color_brewer(palette="Dark2")+ scale_x_sqrt()
smr_bias_ce_black_scatter_wo_oulier + facet_wrap(~variable) + 
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE)+ ggtitle("SMR bias vs ce_exp_counts(only black)")
...
smr_mape_bw_log10 <- ggplot(melt(smr_mape_box), aes(x=race,y=value, fill=variable)) + 
    geom_boxplot() + scale_y_log10()+ylab("MAPE(log10 scale)") + ggtitle("MAPE for SMRs between NHW & Black")
smr_mape_bw_log10

smr_mape_bw <- ggplot(melt(smr_mape_box), aes(x=race,y=value, fill=variable)) + 
    geom_boxplot()+ylab("MAPE") + ggtitle("MAPE for SMRs between NHW & Black")
smr_mape_bw
...
```{r histogram of the exp(theta) by race}
dim(true_smrs_dp20)
hist_exp_theta <- cbind(adat[2],c(t(true_smrs_dp19)))
names(hist_exp_theta) <- c("race","exp_theta")

p <- ggplot(data = hist_exp_theta, mapping = aes(
  x = exp_theta))+ scale_x_log10() +ggtitle("19 DP simulation set") +xlab("exp_theta(log10 scale)")
p + geom_histogram() +
  facet_wrap(~ race) 
```

analogous plots for the bias (rather than MAPE) comparing census/dp/dp0527 to exp(theta)
...
smr_bias_bw <- ggplot(melt(smr_bias_box), aes(x=race,y=value, fill=variable)) + 
    geom_boxplot()+ylab("Bias") + ggtitle("Bias for SMRs between NHW & Black")
smr_bias_bw
...
smr_mape_bias
#grid.arrange(arrangeGrob(smr_mape_bias, bottom = x.grob))
pdf('smr_mape_bias.pdf',width=7,height=6)
print(smr_mape_bias)
```
...
```{r smr_bias_mape}
require(reshape)
smr_bias_mape <- cbind(adat[1:2], bias_ij_ce, bias_ij_dp19, bias_ij_dp20, mape_ij_ce, mape_ij_dp19, mape_ij_dp20) 
names(smr_bias_mape) <- c("GEOID", "race","Bias_ij_CE","Bias_ij_DP19","Bias_ij_DP20","MAPE_ij_CE","MAPE_ij_DP19","MAPE_ij_DP20")
smr_bias_mape_long <- melt(smr_bias_mape)
#generate necessary column names
colmn <- c("metric","DataSources")
smr_bias_mape_long <- cbind(smr_bias_mape_long, reshape2::colsplit(smr_bias_mape_long$variable, "_ij_", colmn))
smr_bias_mape_bw_woo <- ggplot(smr_bias_mape_long, aes(x=race,y=value, fill=DataSources)) + 
    geom_boxplot(outlier.shape = NA) +
  facet_wrap(.~metric,scales="free_y") +ylab("")+xlab("Race")
smr_bias_mape_bw_woo
# scales="free_y"
```
