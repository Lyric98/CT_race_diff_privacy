...
# # scatter plot of expected counts (ce vs. dp)
# ce_exp <- data.frame(t(ce_matrix)) %>% gather(race, exp_counts_ce)
# dp_exp <- data.frame(t(dp_matrix)) %>% gather(race1, exp_counts_dp)
# df <- cbind(ce_exp, dp_exp)
# scatter <- ggplot(data = df, mapping = aes(x = exp_counts_ce, y = exp_counts_dp)) + geom_point() 
# scatter <- scatter + facet_wrap(~race, scales="free") + geom_abline(colour="red")
...
# sum(adat$exp_counts_ce <= 0)
# 796
# sum(adat$exp_counts_dp <= 0)
# 1574
# sum(adat$acs_pov_pct <= 0)
# 35
# dim(adat)
# 10220     5
